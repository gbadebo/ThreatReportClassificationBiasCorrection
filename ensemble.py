...
class Ensemble(object):
	...
	"""
	def __addModel(self, model, data, maxvar):
		index = 0
	...
	"""
	def generateNewModel(self, srcData, srcLabels, trgData, weightSrcData, svmC, svmGamma, svmKernel):
		model = Model()
		...
		model.trainUsingWeights(srcData, srcLabels, weightSrcData, Properties.MAXVAR, svmC, svmGamma, svmKernel)

		#compute source and target weight
		Properties.logger.info('Computing model weights')
		model.weight = model.computeModelWeight(trgData, Properties.MAXVAR)

		#update ensemble
		index = self.__addModel(model, trgData, Properties.MAXVAR)
		if index != -1:
	...
	def evaluateEnsemble(self, dataInstance):
		confSum = {}
		weightSum = {}
		for m in self.models:
